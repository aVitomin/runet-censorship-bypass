...
Текущий опубликованный `v0.0.3.0` содержит только Chromium-пакет. Firefox станет
обычным пользовательским продуктом после Mozilla signing совместного выпуска.
